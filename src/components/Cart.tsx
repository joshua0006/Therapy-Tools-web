...
const Cart: React.FC<CartProps> = ({ className }) => {
  ...
  // Don't render anything if not loaded or cart is closed and not animating
  if (!isLoaded || (!isCartOpen && !isAnimating)) return null;
  ...
  return (
    <>
      ...
      <div 
        ref={cartRef}
        className={`fixed right-0 top-0 h-full w-full md:w-96 bg-white shadow-xl z-50 flex flex-col transition-transform duration-300 ease-in-out ${
        ...
        <div className="flex-1 overflow-auto p-4">
          {cartItems.length === 0 ? (
            <div className="text-center py-8">
              ...
              <button 
                className="mt-4 text-[#2bcd82] hover:underline"
                onClick={() => {
                  closeCart();
                  navigate('/catalog');
                }}
